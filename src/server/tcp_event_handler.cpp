...
namespace net 
{
EventHandler::EventHandler(std::ifstream& a_configStream, std::ostream& a_logStream, size_t a_numOfThreads)
...
void EventHandler::HandlerIncoming(std::shared_ptr<TCPClientSocket>& a_client)
{
    ...
    m_threadPool.AddTask(AbstractActPtr{new threads::HandlerAct{a_client, m_eventPublisher, m_eventPublisherProxy, m_agentManager, m_protocol, m_logStream, bytesVec}});
}
