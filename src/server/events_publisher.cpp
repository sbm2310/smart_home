...
EventsPublisher::EventsPublisher(std::ostream& a_logStream ,size_t a_numOfThreads) 
...
void EventsPublisher::SubmitNotify(Event const& a_event, std::shared_ptr<IEventsPublisher> a_proxy) 
{
    ...
    // auto ptr = std::make_unique<NotifyAct>(eventPtr, m_eventAgentsMapPtr, a_proxy, m_threadsPool, m_logStream);
    // m_threadsPool->AddTask(std::move(ptr));
...
EventsPublisher::NotifyAct::NotifyAct(PtrEvent a_eventPtr, PtrEventAgentsMap a_eventAgentsMapPtr, PtrProxy a_proxyPtr, PtrThreadsPool a_threadsPool, std::ostream& a_logStream)
...
void EventsPublisher::NotifyAct::Act()
{
    ...
    try {
        ...
        for (auto const& agent : a_value) {
            // auto ptr = std::make_unique<InformAgentAct>(m_eventPtr, std::make_shared<IAgent>(agent), m_eventAgentsMapPtr);
            // m_threadsPool->AddTask(std::move(ptr));
